...
CURATION: dict[int, Curated] = {
    ...
    64: Curated(
        ...
        manifest="qlog",
        licence="GPL-3.0",
        note="Parrot's backports carry 0.52.0, ahead of AHRL's 0.49.1; the manifest builds from a tag and says only Kali packages it, which this probe contradicts",
    ),
    ...
    71: Curated(
        ...
        licence="GPL-3.0",
        note="Parrot packages `wsjtx-improved` (2.8.0, a minor behind AHRL's 3.1.0) where the manifest fetches SourceForge's vendor `.deb` and knows only the archive's plain `wsjtx`",
    ),
    ...
    85: Curated(
        ...
        licence="GPL-3.0-or-later",
        note="AHRL installs it with pipx and the manifest hash-pins a venv, saying no distribution packages it; Parrot does, at 0.7.3",
    ),
