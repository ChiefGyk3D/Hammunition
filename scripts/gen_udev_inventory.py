...
`--check` renders and compares instead of writing, so the no-op test can run
it. The "Swept" date is the sweep file's own modification time: the archive
was measured when the sweep ran, not when this page was last regenerated,
and until 2026-09-07 the header claimed the latter (D-031).
"""
...
def read_rows(path: Path, *, enabled_only: bool = True) -> list[dict[str, str]]:
    ...
    if seen and not rows:
        raise SystemExit(
            f"{path.name} has {seen} rows and none with {len(FIELDS)} columns. The "
            f"probe's format changed; update FIELDS rather than reading nothing."
        )
    return rows
...
def render(
    target: str, rows: list[dict[str, str]], held: set[tuple[str, str]], *, swept: date
) -> str:
    ...
    lines = [
        ...
        "",
        f"**Swept:** {swept.isoformat()}, `{target}`  ",
...
def _without_swept(text: str) -> list[str]:
    # A colleague's copy of the same sweep carries a different mtime; the
    # rows are what the check is for.
    return [line for line in text.splitlines() if not line.startswith("**Swept:**")]
...
def main() -> int:
    ...
    if not rows:
        ...
        return 1
    swept = date.fromtimestamp(probe.stat().st_mtime)
    body = render(target, rows, catalog_identifiers(), swept=swept)
    if args.check:
        if not OUT.exists() or _without_swept(OUT.read_text()) != _without_swept(body):
