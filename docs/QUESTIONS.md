...
| `rf_exposure_calc` | Do we ship browser bookmarks? | ✅ **RESOLVED 2026-08-30 — retire, with the other bookmarks.** The URLs live once, as prose, in `docs/guides/propagation.md`. |
| `solar_data` | Same, plus a deprecated ImageMagick dependency | ✅ **RESOLVED 2026-08-30 — retire.** The N0NBH banners are linked from `docs/guides/propagation.md`, without the wget-and-deprecated-`display` plumbing. |
| `xwefax` vs fldigi | Superseded by fldigi's WEFAX mode? | ✅ **RESOLVED 2026-08-30 — carry both, no supersede.** Whether fldigi's built-in WEFAX truly replaces the dedicated tool needs a radiofax operator's judgment; until one weighs in, both stay. |
| `jtdx` | Deprecate in favour of the others? | ✅ **RESOLVED 2026-08-30 — carry as-is, no deprecation mark.** Devoted user base, builds and confirms on every target; demoting on vibes rather than maintenance data is the inherited-verdict mistake. |
| FT8-family default | wsjtx / wsjtx-improved / jtdx / mshv / js8call — which is recommended? | ✅ **RESOLVED 2026-08-30 — `wsjtx`.** Reference implementation, packaged on every target, the documentation everyone else's assumes. Recorded in the wsjtx manifest's own docs; the others stay carried, js8call is a different mode. |
| `FoxTelem` | Enough of the Fox constellation alive to justify it? | ✅ **RESOLVED 2026-08-30 — stage post-1.0 pending an AMSAT status check.** CARRY, manifest deferred until the constellation census says which spacecraft the decoder still serves. |
