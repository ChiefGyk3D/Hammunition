...
Demodulates and modulates AX.25 packet in software from a sound card, replacing a hardware TNC entirely. It handles 1200 baud AFSK and 9600 baud FSK, presents a KISS interface over a pseudo-terminal or a network port, keys the transmitter through a serial line, GPIO or VOX, and includes an APRS digipeater and internet gateway of its own.

## Why you would want it

It made hardware TNCs optional. A radio, a cheap sound interface and this is a complete packet station, and its decoder is measurably better than most of the hardware it replaces -- it runs several demodulators in parallel and takes whichever decodes. Everything else in the packet stack, from Xastir to LinBPQ to Pat, connects to it over KISS.
...
John Langner, WB2OSZ. GitHub issues, and the user guide in the repository is unusually thorough -- it explains packet radio, not just the program.

*Source: [`catalog/packages/direwolf.yaml`](../../catalog/packages/direwolf.yaml)*
