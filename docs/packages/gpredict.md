...
Tracks satellites in real time from Keplerian elements: shows them on a map and in a polar plot, predicts passes with elevation and duration, and drives a rotator and a radio through hamlib so the antenna follows the satellite and the receiver follows the Doppler shift.

## Why you would want it

Working a satellite is a ten-minute window that arrives on a schedule you have to know in advance, with a frequency that moves several kHz while you are on it. Gpredict is what answers both. The Doppler tuning in particular is the difference between chasing a signal across the band and staying on it.
...
Alexandru Csete, OZ9AEC -- also the author of gqrx. GitHub issues linked from the site.

*Source: [`catalog/packages/gpredict.yaml`](../../catalog/packages/gpredict.yaml)*
