...
> Station logs, contest logging and award tracking
...
**Disk footprint:** Around 700 MB installed, and **QLog is a source build with roughly twenty Qt6 development packages and a long compile** — expect that step to dominate the install time. CQRLOG's MariaDB server is most of the disk.

## Why these belong together

Most operators want one logger, not five. They are grouped rather than chosen for you because the right one depends on how you operate, and the difference is real: **QLog** is the most actively developed and the one whose online-service integrations keep working, and it is the recommended default; **CQRLOG** wants a database server and gives award tracking that is actually correct; **xlog** opens in under a second and does the basics; **KLog** sits between them and runs on Windows too; **PyQSO** is the one whose source you can read. TQSL is not a choice — LoTW accepts nothing it has not signed.

## Packages (11)

[`qlog`](../packages/qlog.md), [`cqrlog`](../packages/cqrlog.md), [`klog`](../packages/klog.md), [`xlog`](../packages/xlog.md), [`pyqso`](../packages/pyqso.md), [`tlf`](../packages/tlf.md), [`tucnak`](../packages/tucnak.md), [`trustedqsl`](../packages/trustedqsl.md), [`fllog`](../packages/fllog.md), [`flnet`](../packages/flnet.md), [`python3-pyhamtools`](../packages/python3-pyhamtools.md)
