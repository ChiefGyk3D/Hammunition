...
| `update` | A report of installed versus the catalog: apt against the local lists, builds against the pin through D-051's attribution; lists' age disclosed, nothing run, upstream not asked | D-010 gave every manifest an update block and nothing read it; measured on the field laptop, its 17 behind-the-pin units are exactly the dry run's builds (**D-053**) |
