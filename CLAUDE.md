...
- `docs/reference/coverage-matrix.md` — AHRL's 95 units against the Debian
  Hamradio Blend's task metapackages as Parrot carries them; generated by
  `scripts/gen_coverage_matrix.py` from a fetched archive probe
...
| Menu findability | Groups are activities in plain words in a newcomer's reading order; a toolkit unit (`menu_submenu`) is one nested submenu under its first category; a generated entry carries `menu_title`, *what it does (command)*; the distribution's own entries are never renamed | GNU Radio's 21 entries sat inline in two submenus and 47 generated entries read `tlf`, `wwl`, `atlc` (**D-054**) |
| Vocabulary grain | 55 fine tags under the eight groups, each the thing a person looks for (APRS, Winlink, ships, aircraft, SSTV & amateur TV); every unit retagged; the menu and the future wiki read the same file | "There is no Ham Radio TV": `listening` held six kinds of decoder in one submenu of 19 (**D-055**) |
...
| Offline data | A `data` install method: pinned, hashed artifacts under the shared prefix, each artifact's size and licence printed in the plan before the confirmation; the operator's selection lives in station config and a missing one defers the data unit, never its reader | Five ETC units and `country_files` were readers of data the catalog had no shape for; ETC downloads 0.7 GB tilesets unverified into `/etc/skel` (**D-049**) |
...
| `update` | A report of installed versus the catalog: apt against the local lists, builds against the pin through D-051's attribution; lists' age disclosed, nothing run; `--upstream` opts in to asking GitHub, git tags, PyPI or a label file whether the pin is current | D-010 gave every manifest an update block and nothing read it; measured on the field laptop, its 17 behind-the-pin units are exactly the dry run's builds (**D-053**) |
...
- "Parrot tracks testing/sid" is not what the archive says. Parrot 7 `echo`
  ...
  again on 2026-09-06 for `docs/reference/coverage-matrix.md`). The churn
  list above is about Debian *testing* and reaches Parrot only when Parrot
  rebases.
