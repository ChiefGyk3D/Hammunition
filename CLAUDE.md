...
Full detail and verification notes: `docs/reference/prior-art.md`.

### Licensing gates

- **Never copy, vendor, fork, or adapt code from `km4ack/73Linux` or
  `km4ack/pi-build`.** Both have no license file (verified: LICENSE,
  LICENSE.md, LICENSE.txt, COPYING all 404; GitHub API reports
  `license: null`). That is all-rights-reserved. Their design patterns may be
  reimplemented clean-room from behavioral description only. If you find
  yourself reading their source to write ours, stop and flag it.
- **EMCOMM Tools Community is Apache-2.0 for code, with a separate restrictive
  notice for The Tech Prepper LLC brand logos and images** (non-commercial,
  attribution, no modification). Code is reusable with attribution. Never copy
  branding assets.
- **FISSURE and RF Swift are GPL-3.0.** Fully reusable. If we link or derive,
  Hammunition's affected components inherit GPL-3.0. Flag this before it
  happens, do not decide it silently.
- **linBPQ has no OSI license**, and **VARA HF/FM/SAT is proprietary paid
  Windows software**. Both are install-time downloads at the user's direction
  only. Never vendor, mirror, or redistribute either. For VARA, ship a Wine
  setup helper at most and make `ardopcf` (MIT) the default free path.
- **AREDN is GPL-3.0 with trademark restrictions.** Do not use the AREDN name
  or marks.
- Copyright on our own work is Renegade Penguin LLC.

### Packaging rules

- **Never add a third-party APT archive to the user's `sources.list`.**
  Specifically never add Kali's archive to Parrot. Kali's taxonomy may be
  copied; its packages may not be installed.
- `debian-hamradio` is already in Parrot's own pool. The `hamradio-*`
  metapackages are the base tier and install with plain apt, no new repo.
- **Never install radioconda, conda, or PyBOMBS.** They create a parallel
  prefix that shadows apt's GNU Radio. `radioconda.yaml` is a coverage
  reference only.
- Source tier order: Parrot/Debian apt, then upstream-maintained .deb or
  official apt repo the user opts into, then build-from-source with a pinned
  git ref and recorded checksum. Never build from source what apt already
  provides.

### Package availability is dynamic, not a static list

Probe at install time. Do not hardcode package names as guaranteed present.
Known-unstable as of 2026-09-06:

- `ax25-tools` removed from Debian testing 2026-09-01 (bug #1143282). Present
  in trixie stable and sid. Parrot tracks testing/sid.
- `gr-gsm` marked for autoremoval from testing 2026-10-12 (bug #1146084).
  Upstream is abandoned. Use `bkerler/gr-gsm` branch
  `maint-3.10_with_multiarfcn` from source.
- `python3-meshtastic` marked for autoremoval from testing 2026-10-03.
  `meshtasticd` was never in Debian; it comes from the openSUSE OBS repo.
- `multimon-ng` in Debian is stale at 1.3.1 against upstream 1.6.0.
- `dump1090-mutability` is dead. Use `wiedehopf/readsb`.
- JS8Call development moved to `JS8Call-improved/JS8Call-improved`. Debian sid
  already tracks the fork.
- SDR++ entered Debian unstable 2026-05-28 with a trixie backport. No longer a
  build-from-source item.

### Security posture

Standard project rules apply and are not negotiable for convenience: no
disabling TLS verification to fetch a source tarball, no unsigned repo, no
`curl | sudo bash` for a third-party installer without a checksum gate, no
world-writable install paths, no weakening Parrot's existing hardening to make
a ham app run.

### Where the survey and the decision record disagree

The section above is the maintainer's constraints block, appended as written.
Two of its lines are measured differently by this repository, and per the
authority rule at the top of this file the decision record wins until the
maintainer says otherwise:

- "Never add a third-party APT archive" reads against **D-040**, which admits a
  third-party archive only when the distribution offers nothing, only with the
  key fingerprint pinned in the manifest, only on the operator typing that
  fingerprint, and removes both files on uninstall. The same block's own
  "official apt repo the user opts into" tier is that decision. Read the rule
  as: no archive is ever added by default or by `--yes`, and Kali's is never
  added to Parrot at all.
- "Parrot tracks testing/sid" is not what the archive says. Parrot 7 `echo`
  ships Debian 13 trixie stable plus a backports suite (**D-038**, measured
  again on 2026-09-06 for the coverage matrix). The churn list
  above is about Debian *testing* and reaches Parrot only when Parrot rebases.
